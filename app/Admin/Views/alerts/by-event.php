...
/**
 * Alert Dashboard - By Event View
 * Alertas filtrados por tipo de evento
 *
 * @var string $event
 * @var array $alerts
 * @var int $total
...
<div class="adm-page-header">
    <div>
        ...
        <h1 class="adm-page-title" style="margin-top:.5rem;">Alertas: <span class="code-tag" style="font-size:1rem;"><?= e($event) ?></span></h1>
        <p class="adm-breadcrumb"><?= (int)$total ?> alerta(s) deste tipo de evento</p>
    </div>
</div>
                ...
                <?php if (empty($alerts)): ?>
                    <tr>
                        <td colspan="6" style="text-align:center;color:var(--adm-muted);padding:2rem;">
                            Nenhum alerta encontrado para este tipo de evento
                        </td>
                    </tr>
                <?php else: ?>
...
<div class="pagination">
    <?php if ($page > 1): ?>
        <a href="/administrator/alerts/by-event/<?= e($event) ?>?page=1">Primeira</a>
        <a href="/administrator/alerts/by-event/<?= e($event) ?>?page=<?= $page - 1 ?>">Anterior</a>
    <?php endif; ?>
    <span>Página <?= $page ?> de <?= $pages ?></span>
    <?php if ($page < $pages): ?>
        <a href="/administrator/alerts/by-event/<?= e($event) ?>?page=<?= $page + 1 ?>">Próxima</a>
        <a href="/administrator/alerts/by-event/<?= e($event) ?>?page=<?= $pages ?>">Última</a>
    <?php endif; ?>
</div>
<?php endif; ?>
